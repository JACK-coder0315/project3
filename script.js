...
d3.csv("all_glu_food.csv", d => ({
  ...
  const scatter = dc.scatterPlot("#scatter-plot .dc-chart")
    ...
    .brushOn(true)
    .on('filtered', () => {
      const filtered = scatterDim.top(Infinity);
      drawTimeSeries(filtered[0]);
    })
  ...
  // Draw selected meal's glucose time series
  function drawTimeSeries(sel) {
    ...
    d3.csv(`glucose_series/${sel.event_id}.csv`, row => ({
      ...
      const x = d3.scaleTime()
        .domain(d3.extent(series, d=>d.timestamp)).range([0, width]);
      const y = d3.scaleLinear()
        .domain([d3.min(series,d=>d.glucose), d3.max(series,d=>d.glucose)])
        .range([height,0]);
