...
d3.csv("added_food.csv", d => ({
  ...
  // 10. Scatter Plot with custom tooltip
  const scatter = dc.scatterPlot("#scatter-plot .dc-chart")
    ...
    .renderTitle(false); // disable default <title>

  // Attach hover tooltips via renderlet
  scatter.on('renderlet', chart => {
    // bind original data to each circle
    const plotData = chart.plotData();
    const circles = chart.svg().selectAll('circle.symbol')
      .data(plotData);

    circles
      .on('mouseover', (event, pd) => {
        const d = pd.data; // original record
        const tooltipHtml = `
