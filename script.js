...
d3.csv("added_food.csv", d => ({
  ...
  dc.scatterPlot("#scatter-plot .dc-chart")
      ...
      .on("renderlet", function (chart) {
        chart.select(".brush").lower();
        chart.select(".brush .selection").style("pointer-events", "none");
        bindScatterTooltips();
      });
...
function bindScatterTooltips() {
  const dots = d3.selectAll("#scatter-plot .symbol");
  console.log("Selected dots:", dots.size()); 
  ...
  dots
    .on("mouseover", function (event, d) {
      ...
      tooltip
        ...
        .html(`
          <strong>Person:</strong> ${d.person || d.data?.person || "Unknown"}<br/>
          <strong>Total Carb:</strong> ${d.total_carb || d.data?.total_carb || 0} g<br/>
          <strong>Protein:</strong> ${d.protein_g || d.data?.protein_g || 0} g<br/>
          <strong>Fat:</strong> ${d.fat_g || d.data?.fat_g || 0} g<br/>
          <strong>Δ Glucose:</strong> ${d.grow_in_glu || d.data?.grow_in_glu || 0} mg/dL
        `);
      console.log("Tooltip HTML:", tooltip.html()); 
    })
    // .on("mouseover", function (event, d) {
    //   console.log("Hover data:", JSON.stringify(d, null, 2)); 
